...
| `wpa_capture_kick` | `seq`, `bssid`, `channel` (1–13), `duration_sec` (5–600, default 90), `deauth_count` (5–200, default 30) | `{"resp":"wpa_capture_kick","seq":N,"status":"started"\|"started_no_kick","duration_sec":N,"deauth_count":N}`. Macro: dispara `wpa_capture` + delay 150ms + `deauth(broadcast)`. | 3.5 |
| `pmkid_capture_kick` | `seq`, `bssid`, `channel`, `duration_sec` (5–600, default 60), `deauth_count` (1–100, default 10) | Análogo ao acima. Macro: `pmkid_capture` + delay + `deauth`. | 3.5 |
| `evil_twin_kick` | `seq`, `ssid`, `password` (opcional), `channel` (1–13), `legit_bssid` (opcional, BSSID do AP legítimo pra deauth nele), `deauth_count` (5–200, default 30) | `{"resp":"evil_twin_kick","status":"started","ssid":"X","kick_fired":bool}`. Macro: `evil_twin_start` + (se legit_bssid) deauth no AP legítimo. | 3.5 |
| `recon_full` | `seq`, `include_lan` (bool, default false) | `{"resp":"recon_full","status":"started","wifi_scan":bool,"ble_scan":bool,"lan_scan":bool}`. Macro: dispara `wifi_scan(passive,all)` + `ble_scan(active,15s)` + (se include_lan e conectado) `lan_scan(3000ms)` em paralelo. | 3.5 |
| `karma_then_twin` | `seq`, `channel` (1–13), `duration_sec` (5–120, default 30), `password` (opcional WPA2) | `{"resp":"karma_then_twin","status":"started","channel":N,"duration_sec":N}`. Macro: roda `karma_start` por N segundos, agrega KARMA_HITs por SSID via hook interno, escolhe top-1, sobe `evil_twin` automático. Resposta final assíncrona: `{"status":"twin_up","ssid":"X","hits":N}` ou `{"status":"no_hits"}` ou `{"status":"twin_failed"}`. | 3.5 |
